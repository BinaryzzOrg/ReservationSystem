...
public class CustomDate {
	...
	public void occupy(CalendarNode date) {
		...
		int yearValue = 0;
        int endDateValue = 0;
        int endDateValueCounter = 1;
        int endCountYear = latestOccupied.getYear();

        int startDateValue = 0;
        int startDateValueCounter = 1;

        int endDate = 0;
        int startDate = 0;

        while(startDateValueCounter < latestOccupied.getMonth()) {
            int month = startDateValueCounter;
            if(startDateValueCounter == 1 || startDateValueCounter == 2)
                month = startDateValueCounter + 12;
            else
                month = startDateValueCounter;

            startDateValue += returnLastDayOfGivenMonth(latestOccupied.getEndingDate().getYear(), month);
            startDateValueCounter++; 
        }

        while(endCountYear < latestOccupied.getEndingDate().getYear()) {
            yearValue += (latestOccupied.getYear() % 4 == 0) ? 365 : 364;
            endCountYear++;
        }

        while(endDateValueCounter < latestOccupied.getEndingDate().getMonth()) {
            int month = endDateValueCounter;
            if(endDateValueCounter == 1  || endDateValueCounter == 2)
                month = endDateValueCounter + 12;
            else
                month = endDateValueCounter;

            endDateValue += returnLastDayOfGivenMonth(latestOccupied.getEndingDate().getYear(), month);
            endDateValueCounter++;
        }

        startDate = startDateValue + latestOccupied.getDate();
        endDate = endDateValue + latestOccupied.getEndingDate().getDate() + yearValue;

        latestOccupied.setNumberOfDays(endDate - startDate);
        latestOccupied.setOriginal(true);
        
		System.out.println("\n+========================================+\n"
				 + ">ACCEPTED RESERVATION!" 
				 + "\n>Starting Date: " + MONTHS[date.getMonth() - 1] + " " + date.getDate() + ", " + date.getYear()
				 + "\n>Ending Date: " + MONTHS[date.getEndingDate().getMonth() - 1] + " " + date.getEndingDate().getDate() + ", " + date.getEndingDate().getYear());
		if(date.getMonth() != date.getEndingDate().getMonth() || date.getYear() != date.getEndingDate().getYear())
	...
	private void occupyHelper(int startingMonth, int endingMonth, CalendarNode date) {
		...
		if(startingMonth < endingMonth) {
			...
			latestOccupied.setAccommodation(date.getAccommodation());
		}
	}
