...
public class CustomCalendar {
	...
	/*
	 * to access the reserved dates list
	 */
	public CustomDate getReservedDates() {
	 ...
	 * error.
	 */
	public void makeReservation(CalendarNode startingDate, CalendarNode endingDate, AccommodationNode accommodation) {
		startingDate.setEndingDate(endingDate);
		startingDate.setAccommodation(accommodation);
		startingDate.setOriginalEndingMonth(endingDate.getMonth());
		if(checkDateValidity(startingDate)) {
		Main.isReserved = true;
		reservedDates.occupy(startingDate);
		}else {
			if(startingDate.getMonth() > 12 || startingDate.getMonth() < 1) {
				System.out.println("\n+============================================================+"
						 + "\n\t\t   [RESERVATION ERROR!]"
						 + "\n POSSIBLE ERRORS:"
						 + "\n-The starting date should be moved later than the ending date."
						 + "\n-The given dates should be within the dates of the month."
						 + "\n-The given month is out of bounds (1-12)"
						 + "\n+============================================================+");
